...
class CropRecommendationLSTM:
    ...
    def load_model(self, model_path):
        """
        Load a pre-trained model, scaler, and label encoder from a given full model path.
        Assumes scaler and label encoder are in the same directory.
        """
        import joblib

        # Extract the directory where model is located
        ...
        if not (os.path.exists(model_path) and 
            ...
            raise FileNotFoundError("Saved model, scaler, or label encoder not found in the specified directory.")
